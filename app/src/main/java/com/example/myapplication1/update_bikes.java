...
public class update_bikes extends AppCompatActivity {
    ...
    private void fetchBikes(String userId) {
        ...
        client.newCall(request).enqueue(new Callback() {
            ...
            public void onFailure(Call call, IOException e) {
                e.printStackTrace();
                runOnUiThread(() ->
                        Toast.makeText(update_bikes.this, "Failed to fetch bikes", Toast.LENGTH_SHORT).show()
                );
            }
                ...
                runOnUiThread(() -> {
                    try {
                        if (responseBody.isEmpty()) {
                            Toast.makeText(update_bikes.this, "Empty response from server", Toast.LENGTH_SHORT).show();
                            return;
                        }
                        ...
                        if (dataArray == null || dataArray.length() == 0) {
                            Toast.makeText(update_bikes.this, "No bikes available", Toast.LENGTH_SHORT).show();
                            return;
                        }
                        ...
                        e.printStackTrace();
                        Toast.makeText(update_bikes.this, "JSON Parsing Error: " + e.getMessage(), Toast.LENGTH_LONG).show();
                    }
                });
            }
        });
    ...
    private void addBikeToLayout(String bikeNameStr,String imagepath, String regNumberStr, int number_of_ratings,
        ...
        View bikeView = LayoutInflater.from(this).inflate(R.layout.update_vehicles, null);
        ...
        Button updateButton = bikeView.findViewById(R.id.update_button);
        String imageUrl = config.BASE_URL + imagepath;
        Glide.with(update_bikes.this)
                .load(imageUrl)
        ...
        updateButton.setOnClickListener(v -> {
            Intent intent = new Intent(update_bikes.this, AddBike.class);
            intent.putExtra("user_id", userId);
            intent.putExtra("bike_id", regNumberStr);
